...
def run(final_data, selected_level_1):
    ...
    try:
        ...
        if selected_level_1 != "All":
            ...
            st.write(f"Data after filtering for selected Level-1: {final_data.shape[0]} rows.")
        
        if final_data.empty:
            st.warning("No data available after filtering. Please check the selected Level-1 and transaction type.")
            return
        ...
        for level in monthly_data["Level-1"].unique():
            ...
            try:
                ...
                if forecast_mean > 1e-6:
                    monthly_data.loc[monthly_data["Level-1"] == level, "revenue_index"] = forecast / forecast_mean
                else:
